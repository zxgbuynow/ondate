...
class User extends Home
{
    ...
    function teaInfo(){
        ...
        $row='';
        foreach ($data as $k=>$v){
            $row.="<tr><td>".$v['room']."</td><td>".$v['goods_name']."</td><td>".$v['num']."位</td><td>呼叫时间：".date('Y-m-d H:i:s')."</td><td><button onclick='finish(".$v['id'].")'>确认</button></td></tr>";
        }
    ...
    function sendmy(){
        ...
        $map['type']=0;
        $map['retime']=null;
        $info=M('calls')->where($map)->field('jsbn,end_time,openid,room')->select();
        if($info) {
            foreach ($info as $k => $v) {
