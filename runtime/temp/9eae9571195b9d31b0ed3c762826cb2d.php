...
<?php endif;  !empty($post_url) || $post_url = U('edit?model='.$model['id'], $get_param); ?>
  <form id="form" action="<?php echo $post_url; ?>" method="post" class="form-horizontal form-center">
    <div class="tab-content"> 
      ...
      <div class="card bg-light mb-3">
        ...
        <div class="card-body">
          ...
          <div class="form-group row cf">
            ...
            <div class="col-sm-8">
              <input type="text" class="text input-large" name="title" value="<?php echo htmlentities((isset($data['title']) && ($data['title'] !== '')?$data['title']:'')); ?>">
            </div>
          </div>
          <div class="form-group row cf toggle-imgs">
            ...
            <div class="mult_imgs col-sm-8">
              <div class="upload-img-view" id='mutl_picture_imgs' style="margin-left:100px;"> <?php if(!(empty($data['imgs']) || (($data['imgs'] instanceof \think\Collection || $data['imgs'] instanceof \think\Paginator ) && $data['imgs']->isEmpty()))): if(is_array($data['imgs']) || $data['imgs'] instanceof \think\Collection || $data['imgs'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['imgs'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <div class="upload-pre-item22"> <img width="100" height="100" src="<?php echo htmlentities(get_cover_url($vo)); ?>" />
                  <input type="hidden" name="imgs[]" value="<?php echo htmlentities($vo); ?>" />
                  <em>&nbsp;</em> </div>
                <?php endforeach; endif; else: echo "" ;endif; ?> <?php endif; ?> </div>
        ...
        <div class="card-body">
          ...
          <div style="margin:15px 0;" class="data-table">
            <table cellspacing="1" cellpadding="0">
              ...
              <?php if(!(empty($cate_list) || (($cate_list instanceof \think\Collection || $cate_list instanceof \think\Paginator ) && $cate_list->isEmpty()))): if(is_array($cate_list) || $cate_list instanceof \think\Collection || $cate_list instanceof \think\Paginator): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cd): $mod = ($i % 2 );++$i;?>
              <tr class="list_tr" data-sort="<?php echo htmlentities($cd['sort']); ?>">
                <td align="center" style="width:14%;padding-left:9%;padding-right: 0"><span class="td_key">分类<?php echo htmlentities(chinese_number($cd['sort'])); ?></span></td>
                <td style="width:70%;padding-left:0;"> <?php if(!(empty($cate_data['first']) || (($cate_data['first'] instanceof \think\Collection || $cate_data['first'] instanceof \think\Paginator ) && $cate_data['first']->isEmpty()))): ?>
                  <select style="margin: 6px 30px" name="cate_first[<?php echo htmlentities($cd['sort']); ?>]" class='class_first' data-sort='<?php echo htmlentities($cd['sort']); ?>' onchange="select_cate(this)">
                    ...
                    <option value="<?php echo htmlentities($vo['id']); ?>" <?php if($vo['id'] == $cd['category_first']): ?>selected<?php endif; ?>><?php echo htmlentities($vo['title']); ?></option>
                                                <?php endforeach; endif; else: echo "" ;endif; ?>
                  </select>
                  <?php if(!(empty($cate_data["second"]) || (($cate_data["second"] instanceof \think\Collection || $cate_data["second"] instanceof \think\Paginator ) && $cate_data["second"]->isEmpty()))): if(is_array($cate_data['first']) || $cate_data['first'] instanceof \think\Collection || $cate_data['first'] instanceof \think\Paginator): $i = 0; $__LIST__ = $cate_data['first'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $fid=$vo['id'];  ?>
                  <select class='class_second_<?php echo htmlentities($cd['sort']); ?>' name="cate_second[<?php echo htmlentities($cd['sort']); ?>]" id="cate_second_<?php echo htmlentities($fid); ?>_<?php echo htmlentities($cd['sort']); ?>" data-count="<?php if(!(empty($cate_data['second'][$fid]) || (($cate_data['second'][$fid] instanceof \think\Collection || $cate_data['second'][$fid] instanceof \think\Paginator ) && $cate_data['second'][$fid]->isEmpty()))): ?><?php echo count($cate_data['second'][$fid]); else: ?>0<?php endif; ?>" style="display:none;">
                  ...
                  <input type='hidden' name="select_cate_second[<?php echo htmlentities($cd['sort']); ?>]" id="select_cate_second_<?php echo htmlentities($cd['sort']); ?>" value='<?php echo htmlentities($cd['category_second']); ?>' />
                  <?php endif; ?> </td>
                <td style="width:10%"><input type="hidden" name="cate_ids[<?php echo htmlentities($cd['sort']); ?>]" value="<?php echo htmlentities($cd['id']); ?>" />
          ...
          <div class="form-group row cf toggle-is_recommend">
            ...
            <div class="col-sm-8">
              <div class="check-item">
                <input type="radio" class="regular-radio" value="0" id="is_recommend_0" name="is_recommend"  <?php if(!(empty($data) || (($data instanceof \think\Collection || $data instanceof \think\Paginator ) && $data->isEmpty()))): if($data['is_recommend'] == '0'): ?>checked="checked" <?php endif; ?> <?php endif; ?> />
                <label for="is_recommend_0"></label>
                否 </div>
              <div class="check-item">
                <input type="radio" class="regular-radio" value="1" id="is_recommend_1" name="is_recommend"  <?php if(!(empty($data) || (($data instanceof \think\Collection || $data instanceof \think\Paginator ) && $data->isEmpty()))): if($data['is_recommend'] == '1'): ?>checked="checked" <?php endif; ?> <?php endif; ?> />
          ...
          <div class="form-group row cf toggle-stock">
            ...
            <div class="col-sm-8">
              <input type="text" class="text input-large" name="tab" value="<?php echo htmlentities((isset($data['tab']) && ($data['tab'] !== '')?$data['tab']:'')); ?>">
        ...
        <div class="card-body">
          ...
          <div class="form-item-spec">
            <div class="form-group row cf">
              ...
              <div class="col-sm-8">
                <input type="number" class="text input-large" name="stock" value="<?php echo htmlentities((isset($data['stock']) && ($data['stock'] !== '')?$data['stock']:0)); ?>">
              </div>
            </div>
            <div class="form-group row cf toggle-market_price">
              <label class="col-sm-2"> <span class="need_flag">*</span>原价 </label>
              <div class="col-sm-8">
                <input type="number" class="text input-large" name="market_price" value="<?php echo htmlentities((isset($data['market_price']) && ($data['market_price'] !== '')?$data['market_price']:'0')); ?>">
              </div>
            </div>
            <div class="form-group row cf">
              <label class="col-sm-2">促销价</label>
              <div class="col-sm-8">
                <input type="number" class="text input-large" name="sale_price" value="<?php echo htmlentities((isset($data['sale_price']) && ($data['sale_price'] !== '')?$data['sale_price']:'0')); ?>">
      ...
      <div class="card bg-light mb-3">
        ...
        <div class="card-body"> <?php echo W('common/Express/html', $data); ?> </div>
      </div>
      <div class="card bg-light mb-3">
              ...
              <div class="form-item cf toggle-parm">
                <div class="data-table">
                  <table cellspacing="1" style="width:auto;">
                    ...
                    <?php if(!(empty($param_lists) || (($param_lists instanceof \think\Collection || $param_lists instanceof \think\Paginator ) && $param_lists->isEmpty()))): if(is_array($param_lists) || $param_lists instanceof \think\Collection || $param_lists instanceof \think\Paginator): $i = 0; $__LIST__ = $param_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pp): $mod = ($i % 2 );++$i;?>
                    <tr id="item-tr">
                      <td><input type="text" class="text" name="goods_param_title[<?php echo htmlentities($pp['id']); ?>]" value="<?php echo htmlentities($pp['title']); ?>" style="width:120px;margin-bottom:0;"></td>
                      <td><input type="text" class="text" name="goods_param_value[<?php echo htmlentities($pp['id']); ?>]" value="<?php echo htmlentities($pp['param_value']); ?>" style="width:120px;margin-bottom:0;"></td>
    ...
    <div class="form-item form_bh" style="text-align:center"> <?php if(!(empty($data['id']) || (($data['id'] instanceof \think\Collection || $data['id'] instanceof \think\Paginator ) && $data['id']->isEmpty()))): ?>
      ...
      <button style="margin-right: 30px"  class="btn submit-btn" type="button" target-form="form-horizontal" onclick="do_submit(1)"><?php echo htmlentities((isset($submit_name) && ($submit_name !== '')?$submit_name:'直接上架')); ?></button>
      <button class="btn submit-btn" style="width:220px" type="button" target-form="form-horizontal" onclick="do_submit(2)"><?php echo htmlentities((isset($submit_name) && ($submit_name !== '')?$submit_name:'仅保存至待上架')); ?></button>
    </div>
  </form>
