    ...
    <div class="tab-content">
      ...
      <form id="form" action="<?php echo htmlentities($post_url); ?>" method="post" class="form-horizontal form-center">
        <?php if(is_array($fields) || $fields instanceof \think\Collection || $fields instanceof \think\Paginator): $i = 0; $__LIST__ = $fields;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;if($field['is_show'] == 4): ?>
            <input type="hidden" class="text" name="<?php echo htmlentities($field['name']); ?>" value="<?php echo I($field['name'], $field['value']); ?>">
          <?php endif; if($field['is_show'] == 1 || $field['is_show'] == 2 || ($field['is_show'] == 5 && I($field['name']))): ?>
            <div class="form-group row cf toggle-<?php echo htmlentities($field['name']); ?>">
              <label class="col-sm-2">
                ...
                <?php echo htmlentities($field['title']); ?> </label>

              <div class="col-sm-8">
                <?php switch($field['type']): case "num": ?>
                    <input type="number" class="text" name="<?php echo htmlentities($field['name']); ?>" value="<?php echo I($field['name'], $field['value']); ?>">
                  <?php break; ?><br />
                  <?php case "string": ?>
                  ...
                  <?php break; case "datetime": ?>
                    <input type="datetime" autocomplete="off" name="<?php echo htmlentities($field['name']); ?>" class="text time" value="<?php echo I($field['name'], $field['value']); ?>" placeholder="请选择时间" />
                  <?php break; case "date": ?>
                    <input type="datetime" autocomplete="off" name="<?php echo htmlentities($field['name']); ?>" class="text date" value="<?php echo I($field['name'], $field['value']); ?>" placeholder="请选择日期" />
                  <?php break; case "bool": $_result=parse_field_attr($field['extra']);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                      <div class="check-item">
                        <!--[if !IE]><!-->
                        <input type="radio" class="regular-radio toggle-data" value="<?php echo htmlentities($key); ?>" name="<?php echo htmlentities($field['name']); ?>" id="<?php echo htmlentities($field['name']); ?>_<?php echo htmlentities($key); ?>"  toggle-data="<?php echo htmlentities(get_hide_attr($vo)); ?>"
                  ...
                  <?php break; case "cascade": ?>
                    ...
                    <?php echo W('common/cascade/cascade', array('name'=>$field['name'],'value'=>$field['value'],'extra'=>$field['extra'])); break; case "dynamic_select": ?>
                  ...
                  <?php echo hook('image', array('name'=>$field['name'],'value'=>$data[$field['name']],'extra'=>$field['extra'])); break; case "radio": $_result=parse_field_attr($field['extra']);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    ...
                    <?php endforeach; endif; else: echo "" ;endif; break; case "checkbox": $_result=parse_field_attr($field['extra']);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                      <div class="check-item"> <input type="checkbox" class="regular-checkbox toggle-data" value="<?php echo htmlentities($key); ?>" id="<?php echo htmlentities($field['name']); ?>_<?php echo htmlentities($key); ?>" name="<?php echo htmlentities($field['name']); ?>[]" toggle-data="<?php echo htmlentities(get_hide_attr($vo)); ?>"
                  ...
                  <?php break; case "picture": ?>
                    <div class="col-sm-8 uploadrow2"  data-max="1" title="点击修改图片" rel="<?php echo htmlentities($field['name']); ?>">
                  ...
                  <?php break; case "mult_picture": ?>
                  	<div class="mult_imgs">
                        ...
                        <div class="col-sm-8 uploadrow2" data-max="9" title="点击上传图片" rel="<?php echo htmlentities($field['name']); ?>">
                          <input type="file" id="upload_picture_<?php echo htmlentities($field['name']); ?>">
                        </div>
                    </div>
                  <?php break; case "file": ?>
                    <div class="col-sm-8 upload_file">
                      <input type="file" id="upload_file_<?php echo htmlentities($field['name']); ?>" title="点击修改文件">
                      ...
                      <div class="upload-img-box">
                        <?php if(isset($data[$field['name']])): ?>
                          <div class="upload-pre-file"><span class="upload_icon_all"></span><?php echo htmlentities($data[$field['name']]); ?></div>
                        <?php endif; ?>
                      </div>
                    </div>
                  <?php break; case "user": ?>
                    <div class="col-sm-8">
                    	<div id="userList" class="common_add_list fl">
                            ...
                            <?php else: ?>
                            <a href="javascript:;" class="common_add_btn fl" onClick="$.WeiPHP.selectSingleUser('<?php echo U ( 'weixin/UserCenter/lists_choose'); ?>','<?php echo htmlentities($field['name']); ?>');"></a>
                            <?php endif; ?>
                       </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <div id="userList" class="common_add_list fl">
                                <?php if(!(empty($data[$field['name']]) || (($data[$field['name']] instanceof \think\Collection || $data[$field['name']] instanceof \think\Paginator ) && $data[$field['name']]->isEmpty()))): 
                                    ...
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                <?php endif; ?>
                               <a href="javascript:;" class="common_add_btn fl" onClick="$.WeiPHP.selectMutiUser('<?php echo U ( 'weixin/UserCenter/lists_choose'); ?>',9,'<?php echo htmlentities($field['name']); ?>');"></a>
                           </div>
                        </div>
                      <?php break; default: ?>
                  <input type="text" class="text input-large" name="<?php echo htmlentities($field['name']); ?>" value="<?php echo I($field['name'], $field['value']); ?>">
