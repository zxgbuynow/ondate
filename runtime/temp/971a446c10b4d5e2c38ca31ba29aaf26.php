        ...
        <!-- 高级搜索 -->
         ...
         <form  class="sleft mr10" action="" method="post">
			              <select name="goods_category" style="padding:3px 4px; margin:0; width:150px; border:none" >
			              	<option value="0" <?php if($goods_category==0)echo 'selected=selected'; ?>>按分类查看</option>
			              	<?php if(!(empty($goods_category) || (($goods_category instanceof \think\Collection || $goods_category instanceof \think\Paginator ) && $goods_category->isEmpty()))): if(is_array($goods_category) || $goods_category instanceof \think\Collection || $goods_category instanceof \think\Paginator): $i = 0; $__LIST__ = $goods_category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
			              			<option value="<?php echo htmlentities($vo['id']); ?>" <?php if($cid==$vo['id']){echo 'selected=selected';} ?> ><?php echo htmlentities($vo['title']); ?></option>
			              		<?php endforeach; endif; else: echo "" ;endif; ?>
			              	<?php endif; ?>
			              </select>
			</form>
            ...
            <?php if($search_button): ?>
                <div class="sleft">
                <?php empty($search_key) && $search_key=$model['search_key'];empty($search_key) && $search_key='title';unset($get_param[$search_key]);  $get_param['model']=$model['name']; $search_url = isset($search_url) ?$search_url : $search_url = U('lists', $get_param); ?>
          ...
          <table cellspacing="1">
            ...
            <tbody>
              <?php if(is_array($list_data) || $list_data instanceof \think\Collection || $list_data instanceof \think\Paginator): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?>
                <tr>
                  <?php if($check_all): ?><td><input class="ids regular-checkbox" type="checkbox" value="<?php echo htmlentities($data['id']); ?>" name="ids[]" id="check_<?php echo htmlentities($data['id']); ?>"><label for="check_<?php echo htmlentities($data['id']); ?>"></label></td><?php endif; if(is_array($list_grids) || $list_grids instanceof \think\Collection || $list_grids instanceof \think\Paginator): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$grid): $mod = ($i % 2 );++$i;if(($grid['raw'] == 1) OR ($grid['come_from'] == 1)): ?>
                    <td>
                        <?php if($grid['come_from'] == 1): ?>
                        <a  target="_self" class="btn" href="<?php echo U('addOrder',array('id'=>$data['id'])); ?>">后台下单</a>
                        <?php endif; ?>
                        <?php echo $data[$key]; ?>
                      </td>
                    <?php else: ?><td><?php echo htmlentities($data[$key]); ?></td><?php endif; ?>
                  <?php endforeach; endif; else: echo "" ;endif; ?>
                </tr>
              <?php endforeach; endif; else: echo "" ;endif; ?>
            </tbody>
...
$(function(){
	...
	$("select[name='goods_category']").change(function(){
		var url = '<?php echo U ( "shop/Goods/lists", array("type"=>I("type"))); ?>?cid='+$(this).val();
		url = change_search_url(url);
		window.location.href = url;
	});
