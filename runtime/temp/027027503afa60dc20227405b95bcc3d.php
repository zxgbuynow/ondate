...
<?php endif; if($add_button || $del_button || $search_button || !empty($top_more_button)): ?>   
      <div class="table-bar">
        ...
        <div class="search-form fr cf">
          <div class="sleft">
          <?php empty($search_key) && $search_key=$model['search_key'];empty($search_key) && $search_key='title';unset($get_param[$search_key]);  isset($model['name']) && $get_param['model']=$model['name']; isset($search_url) || $search_url = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/lists', $get_param); ?>
          ...
          <table cellspacing="1">
            ...
            <tbody>
              <?php if(is_array($list_data) || $list_data instanceof \think\Collection || $list_data instanceof \think\Paginator): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?>
                <tr>
                  <?php if($check_all): ?><td><input class="ids regular-checkbox" type="checkbox" value="<?php echo htmlentities($data['id']); ?>" name="ids[]" id="check_<?php echo htmlentities($data['id']); ?>"><label for="check_<?php echo htmlentities($data['id']); ?>"></label></td><?php endif; ?>
                  <!--
                  <?php if(is_array($list_grids) || $list_grids instanceof \think\Collection || $list_grids instanceof \think\Paginator): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$grid): $mod = ($i % 2 );++$i;if(($grid['raw'] == 1) OR ($grid['come_from'] == 1)): ?><td><?php echo $data[$key]; ?></td><?php else: ?><td><?php echo htmlentities($data[$key]); ?></td><?php endif; ?>
                  <?php endforeach; endif; else: echo "" ;endif; ?>
                  -->
                  <td><?php echo htmlentities($data['card_no']); ?></td>
                  <td>
                      <?php echo htmlentities($data['phone']); ?>
                  </td>
                  <td><?php echo htmlentities($data['money']); ?></td>
                  <td><?php echo htmlentities(date("Y-m-d H:i:s",!is_numeric($data['last_time'])? strtotime($data['last_time']) : $data['last_time'])); ?></td>
                  <td>

                      <a href="<?php echo U('edit',array('id'=>$data['id'],'model'=>$model['id'])); ?>">编辑</a>
                      <a href="<?php echo U('del',array('id'=>$data['id'],'model'=>$model['id'])); ?>">删除</a>
 
<!--                       <a class="shop_qrcode" data-id="<?php echo htmlentities($data['id']); ?>" href="javascript:;">门店二维码</a> -->
                  </td>
                </tr>
              <?php endforeach; endif; else: echo "" ;endif; ?>
            </tbody>
...
$(function(){
	...
	$('.shop_qrcode').click(function(){
		...
		var url = "<?php echo U('sence_qr'); ?>?id="+id;
